...
public abstract class Enemy : MonoBehaviour
{
    ...
    {
        // Prioritize staying on the platform
        if (CheckForGroundAhead(transform.position))
        {
            // Follow the player if within the line of sight and ground is ahead
            ...
            if (distanceFromPlayer < lineOfSight)
            {
                ...
                // Check if moving towards the player will lead to a fall
            ...
            {
                // Continue patrol if not following the player
                MoveTowardsCurrentPatrolPoint();
            }
        }
        else
        {
            // Decelerate if no ground ahead to avoid falling
            DecelerateMovement();
        }
    }
    ...
    protected virtual void MoveTowardsCurrentPatrolPoint()
    {
        ...
        // Check if there is ground ahead in the direction of movement
        bool isGroundAhead = CheckForGroundAhead(nextPosition);
        ...
        if (isGroundAhead)
        {
            ...
            if (direction != Vector2.zero)
            {
                ...
                {
                    transform.localRotation = direction.x < 0 ? Quaternion.Euler(0, 0, 0) : Quaternion.Euler(0, 180, 0);
                }
                else
                {
        ...
        {
            // If no ground ahead, move to the ledge or next patrol point
            Vector2 ledgePosition = FindLedgePosition(direction);
            if (ledgePosition != Vector2.zero)
            {
                currentPatrolPoint.position = ledgePosition;
            }
            else
            {
                CyclePatrolPoints();
